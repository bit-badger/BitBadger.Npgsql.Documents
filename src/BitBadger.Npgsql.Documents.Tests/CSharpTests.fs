    ...
    testList "Integration" [
        testList "Configuration" [
            ...
            test "Serializer returns configured serializer" {
                Expect.isTrue (obj.ReferenceEquals(FS.Configuration.defaultSerializer, Configuration.Serializer()))
        ...
        testList "Document.Custom" [
            testList "Single" [
                ...
                testTask "succeeds when a row is not found" {
                    ...
                    let! doc =
                        Custom.Single($"SELECT data FROM {Db.tableName} WHERE data ->> 'Id' = @id",
                                      Seq.singleton (Tuple.Create ("@id", Sql.string "eighty")), FromData<JsonDocument>)
                    Expect.isNull doc "There should not have been a document returned"
                }
            ]
            testList "List" [
